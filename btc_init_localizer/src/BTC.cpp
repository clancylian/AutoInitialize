...
void read_parameters(rclcpp::Node &nh, ConfigSetting &config_setting, int isHighFly)
{
  if(!isHighFly)
  {
    ...
    config_setting.voxel_size_ = 0.5;
    ...
    config_setting.descriptor_near_num_ = 20;
    ...
    config_setting.similarity_threshold_ = 0.85;
  ...
  nh.get_parameter_or<int>("parallel_stl_enable", config_setting.parallel_stl_enable_, 1);
  nh.get_parameter_or<int>("parallel_stl_min_size", config_setting.parallel_stl_min_size_, 256);
  if (config_setting.parallel_stl_min_size_ < 1) {
    config_setting.parallel_stl_min_size_ = 1;
  }
}

double binary_similarity(const BinaryDescriptor &b1,
                         const BinaryDescriptor &b2) {
  const double denom = static_cast<double>(b1.summary_) + static_cast<double>(b2.summary_);
  if (denom <= 1e-6) {
    return 0.0;
  }
  double dis = 0;
...
{ // step1, voxelization and plane dection
  ...
  if (proj_plane_list.size() == 0) 
  {
    ...
    std::sort(proj_plane_list.begin(), proj_plane_list.end(), plane_greater_sort);
    merge_plane(proj_plane_list, merge_plane_list);
    std::sort(merge_plane_list.begin(), merge_plane_list.end(), plane_greater_sort);
  }
    ...
    std::unordered_map<BTCVOXEL_LOC, BTCOctoTree *> &voxel_map) 
{
  ...
  const bool use_parallel =
      (config_setting_.parallel_stl_enable_ != 0) &&
      (index.size() >= static_cast<size_t>(config_setting_.parallel_stl_min_size_));

  auto init_fn = [&](const size_t idx) {
    iter_list[idx]->second->init_octo_tree();
  };

  if (use_parallel) {
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), init_fn);
  } else {
    std::for_each(index.begin(), index.end(), init_fn);
  }
    ...
    std::vector<BTCPlane *> &project_plane_list) 
{
  ...
  if (origin_list.size() == 1) {
    origin_list[0]->id_ = 1;
    project_plane_list.push_back(origin_list[0]);
    return;
  }
    ...
    std::vector<STDMatchList> &candidate_matcher_vec) 
{
  ...
  std::vector<unsigned char> useful_match(current_STD_list.size(), 0);
  ...
  auto match_fn = [&](const size_t i) {
    const STD &descriptor = current_STD_list[i];
    STD_LOC position;
  ...
  if (use_parallel) {
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), match_fn);
  } else {
    std::for_each(index.begin(), index.end(), match_fn);
  }
  ...
  for (size_t i = 0; i < useful_match.size(); i++) 
  {
    if (useful_match[i]) 
    {
      ...
      {
        const auto db_iter = data_base_.find(useful_match_position[i][j]);
        if (db_iter == data_base_.end()) {
          continue;
        }
        const auto &db_matches = db_iter->second;
        const size_t match_idx = useful_match_index[i][j];
        if (match_idx >= db_matches.size()) {
          continue;
        }
        const int frame_number = db_matches[match_idx].frame_number_;
        if (frame_number < 0 || frame_number >= static_cast<int>(match_array.size())) {
          continue;
        }
        match_array[frame_number] += 1;

        Eigen::Vector2i match_index(i, j);
        index_recorder.push_back(match_index);
        match_list_index.push_back(frame_number);
      }
    }
  }
  ...
  for (int cnt = 0; cnt < config_setting_.candidate_num_; cnt++) 
  {
    ...
    int max_vote_index = static_cast<int>(std::max_element(match_array.begin(), match_array.end()) - match_array.begin());
    ...
    if (max_vote_index >= 0 && max_vote >= 5) 
    {
      ...
      for (size_t i = 0; i < index_recorder.size(); i++) 
      {
        ...
        {
          const size_t query_idx = static_cast<size_t>(index_recorder[i][0]);
          const size_t match_offset = static_cast<size_t>(index_recorder[i][1]);
          const auto db_iter = data_base_.find(useful_match_position[query_idx][match_offset]);
          if (db_iter == data_base_.end()) {
            continue;
          }
          const auto &db_matches = db_iter->second;
          const size_t db_idx = useful_match_index[query_idx][match_offset];
          if (db_idx >= db_matches.size()) {
            continue;
          }
          std::pair<STD, STD> single_match_pair;
          single_match_pair.first = current_STD_list[query_idx];
          single_match_pair.second = db_matches[db_idx];
    ...
    std::vector<std::pair<STD, STD>> &sucess_match_list, pcl::PointCloud<pcl::PointXYZINormal>::Ptr pl_cur) 
{
  ...
  std::vector<size_t> index(static_cast<size_t>(use_size));
  std::iota(index.begin(), index.end(), 0);
  ...
  std::vector<VerifyResult> verify_results(static_cast<size_t>(use_size));

  const bool use_parallel =
      (config_setting_.parallel_stl_enable_ != 0) &&
      (index.size() >= static_cast<size_t>(config_setting_.parallel_stl_min_size_));

  auto verify_fn = [&](const size_t i) {
    const auto &single_pair = candidate_matcher.match_list_[i * static_cast<size_t>(skip_len)];
    int vote = 0;
    ...
    verify_results[i].vote = vote;
    verify_results[i].rot = test_rot;
    verify_results[i].t = test_t;
  };

  if (use_parallel) {
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), verify_fn);
  } else {
    std::for_each(index.begin(), index.end(), verify_fn);
  }
  ...
  for (size_t i = 0; i < verify_results.size(); i++) {
    if (max_vote < verify_results[i].vote) {
      max_vote_index = static_cast<int>(i);
      max_vote = verify_results[i].vote;
      best_rot = verify_results[i].rot;
      best_t = verify_results[i].t;
    }
  }
...
void STDescManager::triangle_solver(const std::pair<STD, STD> &std_pair,
  ...
  // For fixed-size 3x3 matrices, Eigen does not allow thin U/V.
